...
class mercusys {
    ...
    async bloqueraEquipo(ip, tipo = "invitado") {
        ...
        await this.page.waitForSelector('.bEpt' + clase + 'Info')
    ...
    async desbloqueraEquipo(mac) {
        ...
        await this.page.waitForSelector('.bEptLHDFInfo ')
        let ok = await this.page.evaluate((mac) => {

            var macs = document.querySelectorAll('.bEptLHDFInfo > .bEptHostInfo  > span');
            let id = null;
